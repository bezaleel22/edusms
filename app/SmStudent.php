...
class SmStudent extends Model
{
    ...
    {
        return $this->hasMany(SmMarkStore::class, 'student_id')->where('exam_term_id', 4)->join('sm_subjects', 'sm_subjects.id', '=', 'sm_mark_stores.subject_id');
    }
    ...
    //     return $this->hasMany(SmAssignSubject::class, 'class_id', 'class_id')->where('section_id', $record->section_id)->where('active_status', 1)->select('class_id', 'section_id');
    // }

    public function studentRecord()
    {
        return $this->hasOne(StudentRecord::class, 'student_id')->where('academic_id', getAcademicId())->where('school_id', Auth::user()->school_id)->where('is_promote', 0)->with('classMates');
    }
    ...
    public static function getResultData($id)
    {
        ...
        $result = SmMarkStore::where('sm_mark_stores.section_id', $student->section_id)
            ...
            ->where('sm_mark_stores.exam_term_id', $exam_type->id)
        ...
        $records = StudentRecord::where('student_records.academic_id', getAcademicId())
            ...
            ->with(['results' => function ($query) use ($student, $exam_type) {
                $query->where('section_id', $student->section_id)
                    ->where('class_id', $student->class_id)
                    ->where('academic_id', getAcademicId())
                    ->where('is_absent', 0)
                    ->where('total_marks', '!=', 0)
                    ->where('exam_type_id', $exam_type->id);
            }])
            ->get();
