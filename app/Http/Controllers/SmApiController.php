...
class SmApiController extends Controller
{
    ...
    public function studentDetails(Request $request)
    {
        try {
            ...
            $students->where('student_records.academic_id', getAcademicId())->groupBy('student_records.student_id');
            $students->select('sm_students.*','student_records.*','sm_classes.*','sm_sections.*','sm_general_settings.school_code','sm_general_settings.session_year');
            ...
            $students->with(array('category' => function ($query) {
    ...
    public function studentDetailsSearch(Request $request)
    {
        ...
        try {
            ...
            if ($request->class != "") {
                $students->where('sm_classes.id', $request->class);
            }
            if ($request->section != "") {
                $students->where('sm_sections.id', $request->section);
            }
            if ($request->name != "") {
    ...
    public function staffView(Request $request, $id)
    {
        ...
        try {
            $staffDetails = SmStaff::where('user_id', $id);
            $staffDetails = $staffDetails->with('roles','designations', 'departments')->first();
