    ...
    public function view(Request $request, $id)
    {
        try {
            ...
            if (!is_null($custom_field_data)) {
                ...
                $feilds = json_decode($custom_field_data, true);
            } else {
                $custom_field_values = null;
            }

            $exam_type = $feilds['Exam Type'];
            $student = SmStudent::getResultData($id);
            if (!is_null($student)) {
                $url = 'https://pdf.llacademy.ng/cache';
                //  $url = 'https://tunnel.beznet.org/cache';
                $response = Http::post($url, $student);
                $previous_timeline = SmStudentTimeline::where('title', $exam_type)->where('staff_student_id', $id)->first();
                if ($response->ok() && ($previous_timeline == "" || $previous_timeline == null)) {
                    $timeline = new SmStudentTimeline();
                    $timeline->staff_student_id = $id;
                    $timeline->type = 'exam';
                    $timeline->title = $exam_type;
                    $timeline->date = date('Y-m-d', strtotime('2024-03-22'));
                    $timeline->description = 'TERMLY SUMMARY OF PROGRESS REPORT';
                    $timeline->visible_to_student = 1;
                    $timeline->file = 'https://pdf.llacademy.ng/download/' . $id;
            ...
            $timelines = SmStudentTimeline::where('staff_student_id', $id)
                ->where('type', 'exam')->where('academic_id', getAcademicId())
                ->where('school_id', Auth::user()->school_id)
                ->get();
            ...
            dd($e->getMessage().'. on line '.$e->getLine() .' in '. $e->getFile());
            Toastr::error($e->getMessage(), 'Failed');
            return redirect()->back();
        }
    }
